        ...
        <div className="scrollbar-[2px] mt-5 flex gap-x-4 overflow-x-auto p-2 scrollbar-thin scrollbar-track-gray-400/20 scrollbar-thumb-[#eee]">
          {categories?.map(({ title, slug }, index) => {
            return (
              <Link
                href={`/blog?cat=${slug}`}
                className={`flex h-12 w-full min-w-[100px] items-center justify-center gap-3 rounded-xl bg-gray-100 px-4 capitalize text-black shadow-sm hover:opacity-90 lg:min-w-[200px]`}
                key={index}
              >
